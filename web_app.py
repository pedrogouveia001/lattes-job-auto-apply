...
from database import (
    register_user, authenticate_user, get_or_create_google_user, update_user_password,
...
# ----------------- Modern Clean LinkedIn CSS Styling -----------------
st.markdown("""
<style>
    ...
    /* Reduce Streamlit top white space */
    .block-container {
    ...
    /* Top Navbar - Clean LinkedIn / SaaS Style */
    .top-nav {
        display: flex;
        align-items: center;
    ...
    .brand-title {
        font-size: 1.25rem;
        font-weight: 800;
        color: #0A66C2; /* LinkedIn corporate blue */
        display: flex;
        align-items: center;
        gap: 8px;
...
# ----------------- Session State & URL Security -----------------
# Ensure no query parameters leak or remain in the URL
...
if st.session_state["user"] is None:
    logo_img_html = f"<img src='data:image/png;base64,{LOGO_B64}' style='width: 64px; height: 64px; border-radius: 12px; margin-bottom: 12px;'>" if LOGO_B64 else "<span style='font-size: 2.5rem;'>💼</span>"

    st.markdown(f"""
    <div style='text-align: center; margin-top: 24px; margin-bottom: 20px;'>
        {logo_img_html}
        <div style='font-size: 2.2rem; font-weight: 800; color: #0A66C2; letter-spacing: -0.5px;'>
            OmniMatch
        </div>
    ...
    with col_main:
        with st.container(border=True):
            ...
            st.caption("Conecte-se com segurança para buscar oportunidades e gerenciar candidaturas.")

            login_tab, reg_tab, pass_tab = st.tabs(["Acesso com Senha", "Criar Conta", "Definir / Redefinir Senha"])
            
            with login_tab:
                u_in = st.text_input("E-mail ou Usuário", key="login_u", placeholder="ex: pedrogouveia001@gmail.com")
                p_in = st.text_input("Senha", type="password", key="login_p", placeholder="Digite sua senha")
                if st.button("Entrar no OmniMatch", type="primary", use_container_width=True):
                    ...
                    if auth_u:
                        ...
                        st.error("Credenciais inválidas. Se você ainda não cadastrou uma senha para sua conta, defina uma na aba 'Definir / Redefinir Senha'.")

            with reg_tab:
                r_name = st.text_input("Nome de Usuário", key="reg_u", placeholder="ex: pedro_gouveia")
                r_mail = st.text_input("E-mail", key="reg_m", placeholder="ex: usuario@email.com")
                r_pass = st.text_input("Senha", type="password", key="reg_p", placeholder="Crie uma senha de acesso")
                if st.button("Criar Nova Conta", use_container_width=True):
                    if r_name and r_mail and r_pass:
                        ok, msg = register_user(r_name, r_mail, r_pass)
                        if ok:
                            st.success(msg + " Você já pode fazer login na aba 'Acesso com Senha'.")
                        else:
                            st.error(msg)
                    else:
            ...
            with pass_tab:
                st.caption("Defina ou redefina sua senha informando seu e-mail cadastrado:")
                rec_mail = st.text_input("Seu E-mail Cadastrado", key="rec_mail", placeholder="ex: pedrogouveia001@gmail.com")
                rec_pass = st.text_input("Nova Senha", type="password", key="rec_pass", placeholder="Digite a nova senha desejada")
                if st.button("Salvar Senha", use_container_width=True):
                    if rec_mail and rec_pass:
                        ok, msg = update_user_password(rec_mail, rec_pass)
                        if ok:
                            st.success(msg)
                        else:
                            st.error(msg)
                    else:
                        st.error("Informe o e-mail e a nova senha desejada.")
    st.stop()
...
# Top Navigation Bar with Logo & User Profile (LinkedIn Corporate Style)
nav_logo_html = f"<img src='data:image/png;base64,{LOGO_B64}' style='width: 36px; height: 36px; border-radius: 6px; border: 1px solid #CBD5E1;'>" if LOGO_B64 else "💼"
...
with nav_left:
    ...
    <div style='display: flex; align-items: center; gap: 12px; padding: 4px 0;'>
        ...
        <div style='display: flex; align-items: center; gap: 8px;'>
            <span style='font-size: 1.35rem; font-weight: 800; color: #0A66C2; letter-spacing: -0.4px;'>OmniMatch</span>
            <span class='brand-badge'>CAREER & JOB MATCHING</span>
        </div>
    </div>
    """, unsafe_allow_html=True)

with nav_right:
    nav_usr, nav_btn = st.columns([2.3, 1])
    with nav_usr:
        st.markdown(f"""
        <div style='display: flex; align-items: center; justify-content: flex-end; gap: 10px; padding-top: 4px;'>
            <div class='user-avatar'>{full_name[:1].upper()}</div>
            <div style='text-align: right; line-height: 1.25;'>
                <div style='font-size: 0.84rem; font-weight: 700; color: #0F172A;'>{full_name.split()[0]}</div>
                <div style='font-size: 0.72rem; color: #64748B;'>{current_user['email']}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    with nav_btn:
        if st.button("🚪 Sair", key="top_nav_logout_btn", use_container_width=True, help="Sair da conta e encerrar sessão"):
            st.session_state["user"] = None
            if len(st.query_params) > 0:
                st.query_params.clear()
            st.rerun()
